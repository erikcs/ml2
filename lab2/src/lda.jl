...
function ldac2docterm(dictfile::AbstractString, documentfile::AbstractString)
    ...
    # could perhaps be a sparse or ragged matrix instead
...
function lda(document_matrix::AbstractArray,
    ...
    # Basic implementation of LDA with Gibbs sampling
    # Using the notation and description from
    ...
    for iter in 1:n_iter
        if verbose
            if iter % 10 == 0
                println("iteration: $iter, elapsed time: $(time() - START)s")
            end
        end
        for doc in 1:D
            for (i, word) in enumerate(doc_word_idx[doc])
                ...
                # Conditional prob
                left = (CWT[word, :] + beta) ./
                        (vec(sum(CWT, 1)) + W * beta)
                right = (CDT[doc, :] + alpha ) ./
                        (vec(sum(CDT, 1)) + T * alpha)
                pz = left .* right
                pz /= sum(pz)
